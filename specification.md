...
In-Summary is a personal, library-based reading and study environment for iPad that imports documents in **PDF**, **EPUB**, and **Markdown**, paginates them horizontally, supports two annotation modes (semantic text highlights and freeform Apple Pencil ink), overlays persistent floating sticky notes, and syncs annotation metadata through a private CloudKit database. Original document binaries stay on the device that imported them; metadata and annotations roam through iCloud.

| Area | Decision |
|---|---|
| Reader surface | SwiftUI shell hosting one format engine per document via `UIViewRepresentable` |
| Pagination | Forced horizontal; never vertical scroll inside a document |
| Annotation layers | Four z-ordered layers (semantic highlights → PencilKit ink → sticky notes → UI overlays) |
| Persistence | SwiftData `@Model` entities with a private CloudKit database; binaries stay local, assets optional |
...
| Performance | No absolute FPS/latency/RAM targets; manual smoothness verification on real hardware |
...
2. **Reflowable content cannot use `pageIndex` as a stable locator.** Markdown and EPUB re-flow with viewport and font size, which makes page numbers unstable. Stable locators are:
   ...
   - **EPUB** → an EPUB 3 **CFI** (Canonical Fragment Identifier) string, fall back to DOM Range serialization if CFI generation fails.
...
2. Skim §3 (Data Model) and §4 (Format Engines) tables — 10 minutes.
...
## 1. Scope, Personas, and Constraints
...
- Read each document with **forced horizontal pagination** (no vertical scroll inside a document).
...
- Sync annotations and metadata (not original binaries, unless explicitly opted in) across the user's own iCloud devices.

### 1.2 Constraints

- **Platform:** iPadOS 17.0+ only. No iPhone, no Mac Catalyst, no visionOS.
- **Connectivity:** Reads, annotations, and library organization work fully offline. CloudKit sync is opportunistic and best-effort.
...
@Model final class DocumentItem {
    ...
    // Opt-in cross-device book availability (Phase 2+). When nil, the
    // document is unavailable on this device.
...
@Model final class StickyNoteEntity {
    ...
    var width: Double = 180.0                // base size; runtime resizable later
...
| `epub` | `{"cfi":String,"fallbackRange":String?}` | Stable across viewport; falls back to serialized Range when CFI generation fails |

Validation rules on load:

- **PDF:** rects must be within `[0,1]`. If out of range or page count changed, highlight is hidden and surfaced in a "review" list.
- **Markdown:** if `contentHash` does not match the current document, highlight is hidden and flagged.
...
|---|---|---|---|
| PDF text highlight | **PDF document space**, normalized 0..1 of page `MediaBox` | Convert to view space via `PDFView.convert(_:to:)` | PDFKit origin is bottom-left |
...
| `(documentId, anchorFormatRaw, anchorPayloadHash)` is unique per app | Insertion guard in the model context | Prevent duplicate highlights from LWW replays |
...
CloudKit uses record-level last-writer-wins. In-Summary applies the following rules:
...
| Two distinct `TextHighlight` rows inserted concurrently with identical anchor hash on the same page | Keep both; dedupe in the UI by `(colorHex, selectedText, anchorHash)`; let user merge or delete |
| Two edits to the same `TextHighlight` row while offline | LWW by `updatedAt`; on next open, surface a "review changes" badge linking to the prior version stored as `previousAnchorPayload: Data?` (added Phase 2) |
| Two distinct ink drawings on the same page on two devices | LWW by `drawingData`'s associated `updatedAt`; the losing device shows a one-time banner: "your freeform ink on this page was replaced by a newer version from another device" |
...
| Pagination | `displayMode = .singlePage`, `displayDirection = .horizontal`, `usePageViewController(true)` | Custom paginator: text → AST → `NSAttributedString` → repeated `NSTextContainer` sized to viewport | CSS columns at 100vw; horizontal `window.scrollTo` controlled by JS |
| Page-change gesture | Built-in `UIPageViewController` swipe | SwiftUI `TabView(selection:)` with `.page(indexDisplayMode: .never)` | JS `window.scrollTo(left = pageIndex * innerWidth)` triggered by Swift |
...
| Surface for ink | `PKCanvasView` overlay per page | `PKCanvasView` overlay per `NSRange`-derived "page" (Phase 1: page index treated as ordinal) | `PKCanvasView` overlay per CFI-resolved viewport segment |
...
- `usePageViewController(true)` provides native horizontal swipe with inertia.
- `autoScales = true` plus per-page zoom is supported via pinch; zoom does not invalidate highlight rects because they are normalized.

### 4.3 Markdown engine

1. Read `.md` from sandbox.
...
6. Persist `[(NSRange, containerIndex)]` per document; expose ordinal `pageIndex` for display only.
7. Highlight lookup is by `NSRange` within `contentHash`-matched content; if the file changed, the highlight is hidden and listed for review.
8. Ink is persisted per ordinal "page" in Phase 1 with the caveat that re-pagination (font size change) shifts drawings; the spec accepts this and records a "reflow safe" path as a Phase 2+ improvement.
...
4. Inject CSS to force horizontal columns at 100vw and disable vertical scroll.
...
1. Persist outgoing page's `drawingData` (PencilKit) and any pending sticky-note edits to SwiftData.
2. Load incoming page's `PageAnnotation` (lazy).
3. Replay `PKDrawing` into the canvas overlay.
...
- **Opt-in upload:** user taps "Make available on my other devices" in the document's context menu. The app:
...
- **Download:** on another device, the library detects a non-nil `contentCKAsset` for a document that is missing locally and offers "Download" with size and progress.
...
|---|---|
| Initialize Xcode project | iPadOS 17+ target; iPad orientations only; iCloud + CloudKit + Background Modes (Remote Notifications) capabilities enabled |
...
| Task | Concrete deliverable |
|---|---|
| PDF reader | `PDFReaderCoordinator` opens PDFs with `singlePage`/`horizontal`/`usePageViewController` |
...
1. Open a 20-page PDF; horizontal swipe changes pages with `PDFView`'s built-in transition.
2. Draw on page 1, swipe to page 2 (blank), draw on page 2, swipe back to page 1 — the original strokes are preserved byte-for-byte.
3. No vertical scroll appears inside the reader.
...
| Task | Concrete deliverable |
|---|---|
| Markdown parser + paginator | `MarkdownParser` + `MarkdownPaginator` produce `[(NSRange, Int)]` pages; rendered in `TabView(selection:)` |
| EPUB safe-unzip | `EPUBSandboxValidator` passes the test corpus (table-driven tests) |
| EPUB CSS injection | 100vw columns, no vertical scroll, JS bridge for page index and text selection |
| Reading progress | `lastReadLocator` (encoded `NSRange` for MD, encoded CFI for EPUB) persists per document |

**Acceptance criteria:**

1. A 5 KB Markdown file paginates into ≥ 1 page at default font size and ≥ 1 page (possibly different count) at +20% font size; no paragraph is split mid-line in either case.
2. An EPUB from the test corpus loads spine chapters in order; horizontal scroll-to-page is smooth; vertical scroll is absent.
3. Resuming a document restores the reader to the exact `NSRange` (MD) or CFI (EPUB).
4. `EPUBSandboxValidator` rejects the malicious EPUB corpus (path traversal, `<script>` injection, disallowed content types).
...
1. A new sticky note appears at the tap location, persists across page changes, and survives rotation and Split View resizes (manual verification at two window sizes).
...
2. Exporting a PDF with ink + highlights + sticky notes produces a valid PDF where the marks are visible at standard readers.
3. Exporting a `.md` summary produces a readable file listing highlights and sticky notes with stable locators.
4. After opting in to cloud availability on device A, the document on device B shows a "Download" affordance, and tapping it produces a byte-identical local copy (`contentHash` matches).
...
| Snapshot tests | Markdown and EPUB page rendering | `swift-snapshot-testing` (optional, Phase 6 decision) |
...
| Two-device offline edits to the same ink drawing silently overwrite | Medium | Medium | LWW with explicit "your drawing was replaced" banner; preserve prior version on device for one sync cycle |
